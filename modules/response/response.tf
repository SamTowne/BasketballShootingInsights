...
resource "aws_lambda_function" "lambda" {
  ...
  runtime          = "python3.8"
  source_code_hash = filebase64sha256("./modules/response/response_payload.zip")
}

data "archive_file" "lambda" {
  type = "zip"
  source_file = "./modules/response/response.py"
  output_path = "./modules/response/response_payload.zip"
}
