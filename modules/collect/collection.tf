...
resource "aws_lambda_function" "lambda" {
  filename         = "${path.module}/collection_payload.zip"
  function_name    = "collection"
  role             = aws_iam_role.lambda_role.arn
  handler          = "collection.lambda_handler"
  runtime          = "python3.8"
  source_code_hash = filebase64sha256("${path.module}/collection_payload.zip")
}
...
data "archive_file" "lambda" {
  type = "zip"
  source_file = "${path.module}/collection.py"
  output_path = "${path.module}/collection_payload.zip"
}
